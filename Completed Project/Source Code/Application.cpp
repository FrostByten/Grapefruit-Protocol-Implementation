...
--
-- PROGRAMMER: Chris Klassen, Jeff Bayntun
--
...
-- PROGRAMMER: Jeff Bayntun
--
-- INTERFACE: void updateTotalMessages();
--
-- RETURNS: void
--
-- NOTES:
-- Removes lines from output based on screen size so that most current output is visible.
----------------------------------------------------------------------------------------------------------------------*/
void updateTotalMessages()
{
	RECT rect;
	...
	while(totalMessage.size() > height - 3 )
		totalMessage.erase( totalMessage.begin() );
